...
LIST_STORE = ['kimmiebabyshop','makmuronline','istanasusu']

START_DATE = '2024-05-16'
END_DATE = '2024-05-16'

PATH_LIST = ['Traffic Blibli/Daily 2024/5-May']
...
for STORE_LIXUS in LIST_STORE:
    for DATA_PATH in PATH_LIST:
        ...
        for files in ALL_FILES:
            date0 = re.search(r'.*?performance (.*) -.*',files)[1]
            date_file = datetime.datetime.strptime(date0.replace('Mei','May'),'%d %b %Y').strftime('%Y-%m-%d')
            if START_DATE <= date_file and END_DATE >= date_file:
                print(files)

                transform_function = FunctionTrafficBlibli(files, STORE_LIXUS,STORE_DOMAIN, STORE_CATEGORY, PLATFORM, PLATFORM_ID)
