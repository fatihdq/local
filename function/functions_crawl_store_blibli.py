...
class FunctionsCrawlStoreBlibli(object):
    ...
    def basicTransform(self,df):
        dtype_mapping = {'crawling_category':'string','crawling_type':'string','filter':'string','region':'string','url':'string','platform':'string',
                'date':'string','rank':'int','page':'int','price':'int','price_before_discount':'int','discount':'float','item_sold':'int','stock':'int','rating_star':'float','rating_count':'int','view_count':'string',
                'product_id':'string','product_name':'string','predicted_brand':'string','product_url':'string','sku':'string',
        ...
        selected_columns = ['crawling_category','filter','crawling_type','region','platform','url','page','date','rank',
                'price','discount','price_before_discount','stock','item_sold','view_count','rating_star','rating_count',
                'product_id','product_name','predicted_brand','sku','product_url','cat_id','cat_name','subcat_id','subcat_name','subsubcat_id','subsubcat_name',
                'shop_id','shop_name','shop_domain','shop_url','is_official','location']
        renamed_columns = {'category_crawl':'crawling_category','keyword_store':'filter','scrapingtype':'crawling_type','region':'region','platform':'platform','referer':'url','page_number':'page', 'timestamp':'date','rank':'rank',
                'price':'price','discount':'discount','price_before_discount':'price_before_discount','stock':'stock','item_sold':'item_sold','view_count':'view_count', 'rating_star':'rating_star','rating_count':'rating_count',
                'product_id':'product_id','product_name':'product_name','brand':'predicted_brand','sku':'sku','product_url':'product_url','cat_id':'cat_id','category':'cat_name','subcat_id':'subcat_id','subcategory':'subcat_name','subsubcat_id':'subsubcat_id','subsubcategory':'subsubcat_name',
                'shop_id':'shop_id','shop_name':'shop_name','shop_domain':'shop_domain', 'shop_url':'shop_url','is_official_store':'is_official','location':'location',}
        ...
        df_handled_price.loc[:,'price_before_discount'] = df_handled_price['price_before_discount'].apply(lambda x: int(x))
        df_handled_price.loc[:,'price'] = df_handled_price['price'].apply(lambda x: int(x))
    ...
    def lookupProduct(self,df,mapping_product):
        def matchProduct(df_match,mps):
            for mp in mps.to_dict('records'):
                if str(mp['id_product_origin']) == str(df_match['product_id']) and str(mp['id_shop_lixus']) == str(df_match['shop_id']):
