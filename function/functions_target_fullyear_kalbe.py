...
class FunctionTargetFullyearKalbe(object):
    ...
    def basicTransform(self,df):
        ...
        selected_columns = ["shop_category","platform","brand_group","date","gmv","page_view","order_cr","aov"]
        columns_str = ["shop_category","platform","brand_group"]
        columns_int = ["gmv","page_view","order_cr","aov"]
        dtype_mapping = {"shop_category":"str","platform":"str","brand_group":"str","date":"str","gmv":"int","page_view":"int","order_cr":"float","aov":"int"}
    ...
    def insertDataTargetFullyear(self,CONN,df,year):
        ...
        query_date = sql.text("""SELECT DISTINCT date FROM target_fullyear_kalbe AS tfk
                            JOIN shop AS s ON tfk.shop_id = s.shop_id
                            WHERE s.shop_id IN ({});
                        """.format(query_shop_id))

        result_date = CONN.execute(query_date).fetchall()
        mapping_date = [date['date'].strftime('%Y') for date in result_date]


        # check if data is already exist or not
        is_exist = False
        if year in mapping_date:
            is_exist = True
        
        if is_exist == False:
            query = sql.text("""INSERT INTO target_fullyear_kalbe ({})
                            VALUES {};
                            """.format(query_col, query_value))
            CONN.execute(query)
            print("Success Insert {} Data To Table".format(year))
        else:
            print("{} Data Already Exist".format(year))