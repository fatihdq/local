...
class FunctionTrafficTokopedia(object):
    ...
    def basicTransform(self,df):
        ...
        rename_column = {'Nama Produk':'product_name',
                        'SKU':'sku',
                        'Pendapatan Bersih Baru':'gmv',
                        'Produk Terjual':'item_sold', 
                        'Produk Dilihat':'page_view', 
                        'Keranjang':'add_cart',
                        'Pesanan':'transaction'}
        selected_columns = ['product_name', 'sku', 'gmv', 'item_sold', 'page_view', 'add_cart','transaction']
        dtype_mapping = {'product_name':'str', 'sku':'str', 'gmv':'int', 'item_sold':'int', 'page_view':'int', 'add_cart':'int','transaction':'int'}
        columns_str = ['product_name', 'sku']
        columns_int = ['gmv', 'item_sold', 'page_view', 'add_cart','transaction']
        ...
        if len(date)>2:
            date0 = date[1][0:4]+'-'+date[1][4:6]+'-'+date[1][6:8] 
        elif len(date)==2:
