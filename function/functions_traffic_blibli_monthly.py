...
class FunctionTrafficBlibliMonthly(object):
    ...
    def basicTransform(self,df):
        if 'PV' in df.columns:
            rename_column = {'Product Name':'product_name', 'Sales':'gmv', 'Item Sold':'item_sold', 'PV':'page_view','Order':'transaction','Customer':'unique_visitor'}
        elif 'Pageview' in df.columns:
            rename_column = {'Product Name':'product_name', 'Sales':'gmv', 'Sold Product':'item_sold', 'Pageview':'page_view','Order':'transaction','Visitor':'unique_visitor'}
        elif 'Page View' in df.columns:
            rename_column = {'Product Name':'product_name', 'Sales':'gmv', 'Sold Products':'item_sold', 'Page View':'page_view','Order':'transaction','Visitor':'unique_visitor'}
        selected_columns = ['product_name', 'gmv', 'item_sold', 'page_view','transaction','unique_visitor']
        columns_str = ['product_name']
        columns_int = ['gmv', 'item_sold', 'page_view','transaction','unique_visitor']
        dtype_mapping = {'product_name':'str', 'gmv':'int', 'item_sold':'int', 'page_view':'int','transaction':'int','unique_visitor':'int'}
        ...
        ## Date extraction
        ############
        date0 = self.file_name.split(' - ')[-2]
        date = datetime.datetime.strptime(date0,'%d %b %Y').strftime('%Y-%m-%d')
        
        ## Input Basic Value
        #####################
        df_basic_value = df_changed_dtype.copy()
        df_basic_value.loc[:,'date'] = date
    ...
    def productMatch(self,df,dmp,db_mapping_brand,shop_id):
            ...
            for mp in dmp:
                if df['product_name'] == mp['product_name']:
                    ...
                    bol = True
            if bol == False:
                new_product['id_product_origin'] = "{}-{}".format(str(self.get_first_char(df['product_name'])),shop_id)
                new_product['id_product_lixus'] = "{}-{}".format(str(self.get_first_char(df['product_name'])),shop_id)
                new_product['product_name'] = df['product_name']
    ...
    def mappingProduct(self,df,db_mapping_brand, db_mapping_product):
        ...
        mapping_product = pd.DataFrame.from_dict(list(df_product.apply(lambda y: self.productMatch(y,list(filter(lambda x: x['product_name'] == y['product_name'],db_mapping_product)),db_mapping_brand,shop_id),axis=1)),orient='columns')
    ...
    def insertSnapshotsTraffic(self,CONN,df,file_date):
        ...
        date = datetime.datetime.strptime(list_date[0],'%Y-%m-%d')
        last_day = calendar.monthrange(date.year, date.month)[1]
        first_date = date.replace(day=1).strftime('%Y-%m-%d')
        last_date = datetime.date(date.year,date.month,last_day).strftime('%Y-%m-%d')
        ...
        if is_exist == False:
            ...
            query_delete = sql.text("""delete from traffic 
                            ...
                            """.format(query_shop_id,first_date,last_date))
                            
            CONN.execute(query_delete)
            print("Delete {} date = {} - {}".format(self.store_domain, first_date, last_date))
