...
class FunctionProductTokopedia(object):
    ...
    def basicTransform(self,df):
        rename_column= {"Product ID":"product_id", 
        ...
        selected_columns = ["product_id","product_name","product_url","sku","price","stock"]
        columns_str = ["product_id","product_name","product_url","sku"]
        columns_int = ["price","stock"]
        dtype_mapping = {"product_id":"str","product_name":"str","product_url":"str","sku":"str","price":"int","stock":"int"}
        ...
        df_selected = df_selected.loc[1:,:].reset_index(drop=True)
        ...
        ## Remive Duplicate product_id
        ################
        df_revomed_product_id = df_null_value.copy()
        df_revomed_product_id['product_id'] = df_revomed_product_id['product_id'].apply(lambda x: x.split('.')[1]if '.' in x else x)

    
        ## Change Datatype
        #####################
        df_changed_dtype = df_revomed_product_id.copy() 
