...
class FunctionsCrawlKeywordShopee(object):
    ...
    def basicTransform(self,df):
        dtype_mapping = {'crawling_category':'string','crawling_type':'string','filter':'string','region':'string','url':'string','platform':'string',
                ...
                'product_id':'string','product_name':'string','predicted_brand':'string','product_url':'string',
        ...
        selected_columns = ['crawling_type','filter','crawling_category','region','url','platform','date','rank','page',
                ...
                'product_id','product_name','predicted_brand','product_url','cat_id','cat_name','subcat_id','subcat_name','subsubcat_id','subsubcat_name',
                'shop_id','shop_name','shop_domain','location','is_official','shop_url']
        renamed_columns = {'crawling_type':'crawling_type','filter':'filter','crawling_category':'crawling_category','region':'region','referer':'url','platform':'platform','date':'date','rank':'rank','page_number':'page',
                'price':'price','price_before_discount':'price_before_discount','discount':'discount','historical_sold':'item_sold',
                'stock':'stock','rating_star':'rating_star','review_count':'rating_count','star_1':'star_1','star_2':'star_2','star_3':'star_3','star_4':'star_4','star_5':'star_5',
                'product_id':'product_id','product_name':'product_name','product_ctime':'product_ctime','brand':'predicted_brand','product_referer':'product_url',
                'cat_id':'cat_id','category':'cat_name','subcat_id':'subcat_id','subcategory':'subcat_name','subsubcat_id':'subsubcat_id','subsubcategory':'subsubcat_name',
        ...
        idx = df_selected[pd.isnull(df_selected['product_id'])].index
        df_selected = df_selected.drop(index=idx).reset_index(drop=True)
