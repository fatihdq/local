...
class FunctionsCrawlStoreTokopedia(object):
    ...
    def basicTransform(self,df):
        ...
        renamed_columns = {'category_crawl':'crawling_category','scrapingtype':'crawling_type','keyword_store':'filter','region':'region','referer':'url','platform':'platform','timestamp':'date','rank':'rank','page_number':'page',
                ...
                'productId':'product_id','productName':'product_name','productUrl':'product_url','cat_id':'cat_id','category':'cat_name','subcat_id':'subcat_id', 'subcategory':'subcat_name','subsubcat_id':'subsubcat_id','subsubcategory':'subsubcat_name',
        ...
        null_str_columns = ['product_name','shop_name','cat_name','subcat_name' ,'subsubcat_name']
        id_columns = ['product_id','shop_id','cat_id','subcat_id','subsubcat_id']
        ...
        for col in null_str_columns:
            df_null_value.loc[:,col] = df_null_value[col].apply(lambda x : re.sub(r"[\"#@;:<>{}`+=~|.!'?]",'',self.removeEmoji(x)) if pd.isna(x) != True else '')
        ...
        for col in id_columns:
            df_changed_dtype[col] = df_changed_dtype[col].apply(lambda x: self.dtypes_id(x))
