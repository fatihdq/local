...
class FunctionTrafficShopeeV2(object):
    def __init__(self,file_name,store_lixus,store_domain,store_category,platform,platform_id):
        ...
        with open('./transformation/mapping/sub_brand_mapping.json','r') as f:
            self.sub_brand_mapping = json.loads(f.read())
    ...
    def getMappingTableFromDB(self,CONN,df):
        ...
        list_shop_domain = df['shop_name'].unique()
        list_product_id = df['product_id'].unique()
        list_product_id_lixus = list(map(lambda x: '{}-{}'.format(self.platform_id,x), list_product_id))

        
        # string query for shop_name
        query_shop_domain = ", ".join(list(map(lambda x:"\'{}\'".format(x),list_shop_domain)))
        query_product_id = ", ".join(list(map(lambda x:"\'{}\'".format(x),list_product_id)))
        query_product_id_lixus = ", ".join(list(map(lambda x:"\'{}\'".format(x),list_product_id_lixus)))
        ...
        platform_id = list(filter(lambda d: d['platform_name'] == self.platform, db_mapping_platform))[0]['platform_id']



        #query to get shop data 
        text_query = sql.text("""SELECT * FROM shop
                                WHERE platform = {} and shop_domain = '{}' and category = '{}' """.format(platform_id ,self.store_domain,self.store_category))
        result_query = CONN.execute(text_query).fetchall()
        ...
        list_shop_id = [dicti['shop_id'] for dicti in list(filter(lambda d: d['shop_domain'] == self.store_domain, db_mapping_shop))]
        ...
        query = sql.text("""SELECT * from product_merchant_platform
                            where product_id in ({})
                            and shop_id in ({})
                            and platform = {}
                            """.format(query_product_id_lixus, query_shop_id, platform_id))  
    ...
    def subBrandMatch(self,predicted_brand,product_name):
        brand = predicted_brand
        is_match = False
        for list_mapping in self.sub_brand_mapping:
            if predicted_brand not in list_mapping['where_clause']['predicted_brand']:
                for wip in list_mapping['where_clause']['include_product_name']:
                    if self.findWholeWord(wip.upper())(product_name.upper()) and is_match == False:
                        if len(list_mapping['where_clause']['exclude_product_name']) != 0:
                            for wep in list_mapping['where_clause']['exclude_product_name']:
                                if not self.findWholeWord(wep.upper())(product_name.upper()):
                                    brand = list_mapping['brand']
                                    is_match = True
                        else:
                            brand = list_mapping['brand']
                            is_match = True
    ...
    def lookupBrand(self,df,mapping_brand):
        ...
        df['predicted_brand'] = df.apply(lambda x: self.subBrandMatch(x['predicted_brand'],x['product_name']),axis=1)
