...
class FunctionTrafficBlibli(object):
    ...
    def basicTransform(self,df):
        ...
        ## Date extraction
        ############
        date0 = re.search(r'.*?performance (.*) -.*',self.file_name)[1]
        date = datetime.datetime.strptime(date0.replace('Mei','May'),'%d %b %Y').strftime('%Y-%m-%d')
