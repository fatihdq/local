...
STORE_LIXUS = 'bayininja'

START_DATE = '2023-04-01'
END_DATE = '2023-04-30'
DATA_PATH = 'Data Traffic_Order/Tokopedia/2023'
...
def skip_row(file_name,sheet):
    df = pd.read_excel(file_name,sheet_name=sheet,header=None)
    idx = df[df.iloc[:,0] == 'Date' ].index
    return idx[0]
...
for files in ALL_FILES:
    ...
    if START_DATE <= date_file and END_DATE >= date_file:
        ...
        transform_function = FunctionTrafficBayininjaTokopedia(files, STORE_LIXUS,STORE_DOMAIN, STORE_CATEGORY, PLATFORM, PLATFORM_ID)
        df = pd.read_excel(files,sheet_name='Traffic Tokopedia',skiprows = skip_row(files,'Traffic Tokopedia'))
        if len(df) != 0:
            ...
            df_traffic = df_lookup[['date','page_view','transaction','item_sold','gmv','product_id','shop_id']]
